...
2. **模块 B：证据强度评分引擎**，输出 0-100 综合评分与免复核状态（✅ 通过 / ✅⚠️ / 待筛选）。
...
1. **分数和状态一律以脚本输出为准**：综合评分（0-100）、免复核状态、稳定分、Gap 分、工龄都由脚本算好，**不要自己心算、估算或调整数字**，也不要自行判定是否通过。
2. **批量筛选优先直接运行** `scripts/batch_screen_resumes.py`，把它输出的表格**原样**呈现给 HR；不要绕过脚本自行打分排序。
...
主入口会依次完成文本抽取、基本信息解析、经历切分、岗位标准化、经历分类、时长计算、重叠标记、可信度评分、稳定性/GAP 评分、异常校验和证据强度评分（含免复核状态）。
...
- `--weights path/to/权重.docx`：**已弃用**，仅保留兼容。评分已改为单一证据强度分，权重不再参与打分。
...
- `recommendation`：`status`（✅ 通过/✅⚠️/待筛选）、`score_100`（0-100 综合评分）、`pass_threshold`（本次阈值）、`p0_remark`（P0 高亮文案，✅⚠️ 时非空）、理由、置信度、目标岗位、强/弱/降权细节。（`result` 为兼容旧键，已置为 status；`score` 为 0-1 内部分。）
...
批量筛选表（脚本自动输出）含：候选人、状态、综合评分、解析置信度、学历、相关经验月数、技能/关键词命中、不匹配项、P0高亮（✅⚠️ 行加粗显示具体 P0）、人工复核原因、50 字以内评语。
...
评分不能只看关键词，要看证据强度。脚本输出 **0-100 综合评分**与**状态标签**：

- `✅ 通过`：评分 ≥ 阈值（默认 75，可配置）且无 P0 → 直接通过、免复核，HR 自愿承担风险。
- `✅⚠️`：评分达标但有 P0 异常 → 仍算通过，但要看 `p0_remark` 高亮（加粗呈现）。
- `待筛选`：评分 < 阈值 → 人工筛选；解析置信度 < 0.6 时即便达标也降为待筛选。

综合评分 = 实质强判据 BASE(45) + 弱判据 + 相关经验厚度 + 稳定/Gap − 降权 − P1/P2 扣分 − P0 软抑制。
实质强判据（影响 BASE）：学历达 JD 要求、中高可信销售经验达标（默认 ≥6 个月）、最近一段相关、销售经历有职责/客户/结果/时长、履历稳定分 ≥ 60 且 Gap 分 ≥ 70。P0 不归零 BASE，只软抑制并触发 ✅⚠️。

弱判据包括客户沟通经验、简历完整、稳定性尚可、业务结果指标、行业或职能可迁移。

降权因子包括低可信相关经历、相关经历久远、时间线异常、近期频繁跳槽、稳定分低、Gap 分低。详见 `references/recommendation_logic.md`。
...
- 评分 ≥ 阈值即免复核通过（✅/✅⚠️，HR 自愿承担风险）；< 阈值进入待筛选人工处理。仍不做自动淘汰。
- 不将性别、年龄、籍贯、婚育、民族、照片、院校歧视等敏感属性纳入评分或排序。
- 年龄只可在候选人主动提供且与履历时间线明显矛盾时作为人工复核提示，默认不参与综合评分。
- 不主动联系候选人，不代发面试或拒绝通知。
...
- 免复核通过的准确率和误放过率：重点监控“一行销售经历被误判为 ✅ 通过”。
